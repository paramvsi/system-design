...
async function loadContent(slug: string): Promise<ProblemContent | null> {
  try {
    const raw = await fs.readFile(
      path.join(process.cwd(), "content", "comparisons", `${slug}.json`),
      "utf8"
    );
...
async function loadCSS(slug: string): Promise<string> {
  try {
    return await fs.readFile(
      path.join(process.cwd(), "public", "styles", "comparisons", `${slug}.css`),
      "utf8"
    );
  } catch {
    return "";
...
export async function generateMetadata({
  ...
  const description = c.blurb || `Comparison: ${c.title}`;
  ...
  return (
    <>
      ...
      <div className="layout" style={accentStyle}>
        <Topbar title={c.title} difficulty={c.difficulty} kind="comparison" />
