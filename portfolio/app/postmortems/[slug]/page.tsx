...
async function loadContent(slug: string): Promise<ProblemContent | null> {
  try {
    const raw = await fs.readFile(
      path.join(process.cwd(), "content", "postmortems", `${slug}.json`),
      "utf8"
    );
...
async function loadCSS(slug: string): Promise<string> {
  try {
    return await fs.readFile(
      path.join(process.cwd(), "public", "styles", "postmortems", `${slug}.css`),
      "utf8"
    );
  } catch {
    return "";
...
export async function generateMetadata({
  ...
  if (!c) return {};
  const title = `${c.title} — Post-mortem`;
  const description = c.blurb || `Post-mortem: ${c.title}`;
  const url = `/postmortems/${slug}`;
  return {
    title,
    description,
  ...
  return (
    <>
      ...
      <div className="layout" style={accentStyle}>
        <Topbar title={c.title} difficulty={c.difficulty} kind="postmortem" />
