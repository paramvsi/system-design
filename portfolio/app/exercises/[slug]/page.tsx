...
async function loadContent(slug: string): Promise<ProblemContent | null> {
  try {
    const raw = await fs.readFile(
      path.join(process.cwd(), "content", "exercises", `${slug}.json`),
      "utf8"
    );
...
async function loadCSS(slug: string): Promise<string> {
  try {
    return await fs.readFile(
      path.join(process.cwd(), "public", "styles", "exercises", `${slug}.css`),
      "utf8"
    );
  } catch {
    return "";
...
export async function generateMetadata({
  ...
  if (!c) return {};
  const title = `${c.title} — Exercise`;
  const description = c.blurb || `Whiteboard exercise: ${c.title}`;
  const url = `/exercises/${slug}`;
  return {
    title,
    description,
  ...
  return (
    <>
      ...
      <div className="layout" style={accentStyle}>
        <Topbar title={c.title} difficulty={c.difficulty} kind="exercise" />
        ...
        <main id="main" className="main">
          ...
          {c.sections.map((sec) => {
            const gated = sec.id === "rubric" || sec.id === "redflags";
            return (
              <div key={sec.id}>
                {gated && (
                  <RevealGate
                    slug={slug}
                    sectionId={sec.id}
                    label={sec.id === "rubric" ? "rubric" : "red flags"}
                  />
                )}
                <ProblemSectionBody section={sec} flow={null} accent={c.accent} />
              </div>
            );
          })}
        </main>
